...
def make_gaussian_filter(radius=3, sigma=1, round_crop=True):
...
def make_difference_of_gaussian(radius=5, sigma_1=1, sigma_2=3, round_crop=True):
    G1 = make_gaussian_filter(radius=radius, sigma=sigma_1, round_crop=round_crop)
    G2 = make_gaussian_filter(radius=radius, sigma=sigma_2, round_crop=round_crop)
    sum1 = sum(sum(G1))
    sum2 = sum(sum(G2))
    G2 = sum1 / sum2 * G2
    DoG = G1 - G2
